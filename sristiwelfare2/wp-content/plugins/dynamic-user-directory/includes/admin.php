  ...
  <?php if ( !in_array( 'dynamic-user-directory-multiple-dirs/dynamic-user-directory-multiple-dirs.php' , $dud_plugin_list )) 
        { ?>
			 <tr>
				...
				<td>Copy and paste this shortcode onto the page where the directory should be displayed. The <a href="http://sgcustomwebsolutions.com/wordpress-plugin-development/" target="_blank">Multiple Directories Add-on</a> will let you create and display up to 50 directories, each with its own settings.</td>
				<td></td>
			 </tr> 
  <?php }
        else 
	...
	<table class="form-table">
		...
		<tr>
			...
			<td><input class="dd-menu-no-chk-box-width" type="text" id="ud_filter_fld_value" name="<?php echo $dud_option_name;?>[ud_filter_fld_value]" 
				value="<?php echo !empty($dud_options['ud_filter_fld_value']) ? esc_attr( $dud_options['ud_filter_fld_value'] ) : ""; ?>" maxlength="50"/></td>
				 ...
				 <tr id="meta_fld_<?php echo $inc; ?>">
					...
					<td><b>Format Meta Field <?php echo $inc; ?> As</b><br>
						<select name="<?php echo $dud_option_name;?>[dud_fld_format_<?php echo $inc; ?>]" id="dud_hyperlink_flds">
							...
							<OPTION value="31" <?php echo (!empty($dud_options['dud_fld_format_' . $inc]) && $dud_options['dud_fld_format_' . $inc] == "31") ? "SELECTED" : ""; ?>>Phone Number (Australian format)</OPTION>							
...
<div class="dud-settings-section">
	<table class="form-table">
			...
			<tr>
				...
				<td>Show or hide the Last Name / Display Name (based on your Sort Field setting) as a search field. If you wish to search on first name, simply enter the standard WordPress "first_name" meta key name below.</td>
				<td></td>
			</tr>
			...
			<?php 
				...
				?> 
					<tr id="meta_srch_fld_<?php echo $inc; ?>">
						...
						<?php if($inc === 1) { ?>
							<td>Each Search Meta Key Name should match one in your existing directory. If the "General Search" box below is not checked, the Search Meta Field Labels will be displayed as options in a dropdown box in the order you entered them here.</td>
						<?php } else { ?>
							<td></td>
						<?php } ?>
			...
			<tr>
				...
				<td>If this is turned on, the meta field selector dropdown will be hidden and DUD will automatically look for the search value within all the meta fields entered above. 
                    Please note: if you have an alpha links directory, the "Show Search Results" setting below will automatically change to "Single Page Format."</td>
				<td></td>
			 </tr>	
