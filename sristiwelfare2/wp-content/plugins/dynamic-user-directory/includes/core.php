...
if(strlen($dud_user_srch_name) > 0) 
{
	if(strlen($dud_user_srch_name) > 50)
		return apply_filters('dud_search_err', "<H4>The search field is limited to 50 characters!</H4><br>");
	
	else if (strlen(trim($dud_user_srch_name)) == 0)
		$srch_err = apply_filters('dud_search_err', "<H4>Please enter a valid search value.<H4><br>");
...
else	
{
	...
	if(count((array)$letters) == 0 && $dud_user_srch_name ) 
	{
		...
		$srch_err = apply_filters('dud_search_err', "<H4>No users were found matching your search criteria.</H4><br>");
	}
}
...
if ( count((array)$letters) == 0 && !$dud_user_srch_name && empty($srch_err))	
	return apply_filters('dud_no_users_msg', "<H4>There are no users in the directory at this time.</H4>");	
...
else
{
	...
	if(!empty($ud_sort_fld_key) && $custom_sort_active)
	{
		...
		if(!is_null($last_name_letter) && !$letter_found && !($ud_directory_type === "all-users"))
			$srch_err = apply_filters('dud_search_err', "<H4>No users for the selected letter were found matching your search criteria.</H4><br>");
	}
...
{	
	...
	if(($ud_show_num_users || $exclude_user_filter_active) && $dud_options['ud_directory_type'] !== "all-users")
	{
...
{ 	
	...
	if ( in_array( 'dynamic-user-directory-horizontal-layout/dud_horizontal_layout.php' , $plugins ))
	{
		...
		if($ud_display_listings === "horizontally" && $srch_err !== "<H4>Please enter a valid search value.<H4><br>")
		{	
	...
	$ud_sort_cat_hide_username     		   		= !empty($dud_options['ud_sort_cat_hide_username']) ? $dud_options['ud_sort_cat_hide_username'] : null;
	...
	if(!empty($ud_txt_after_num_users_srch) && endswith($ud_txt_after_num_users_srch, "results") && $total_users === 1)
	...
	{   		
	   ...
	   {	
		...
		if($user_fullname !== '' && !($custom_sort_active && $ud_sort_cat_hide_username === "hide"))
	...
	{		
				...
				$ud_sort_show_cats_dd_hide_dir_before_srch === 'no')
		{
			...
			$user_contact_info .= dud_get_basic_srch_form_html($custom_sort_active, $dud_options);
			$user_contact_info .= "</form><BR>\n"; 
			$user_contact_info .= apply_filters('dud_search_err', "<H4>No users were found matching your search criteria.</H4><br>");
		}
		else
			$user_contact_info .= apply_filters('dud_search_err', "<H4>No users were found matching your search criteria.</H4><br>");
	}		
...
else // No uids were found
{
	...
	if((!empty($dud_user_srch_name) || !empty($search_category)) || $srch_err === "<H4>Please enter a valid search value.<H4><br>" ||
	...
	{                                     							       //show the meta fld search box, for those who only want to show the directory when a search is run
		...
		{
			$srch_err = "<H4>No users were found matching your search criteria.</H4><br>";
		}		
...
function dynamic_ud_format_meta_val($user_meta_fld, $dud_options, $format)
{
	...
	**********************************************/
	...
	//Phone
	if(!is_array($user_meta_fld) && (($format === "6") || ($format === "31") || ($format === "32")))
	{
...
function formatAustralianPhoneNumber($phone) 
{
	try
	{
		if(!empty($phone) && strlen($phone) == 10)
		{
			...
			$phone = $firstFour . " " . $nextThree . " " . $lastThree;
		}
...
function dynamic_ud_build_roles_query($sort_fld, $ud_hide_roles, $ud_roles_exc_inc = null) {
	...
	if(!empty($ud_roles_exc_inc) && $ud_roles_exc_inc === "include")
	{
		$roles_sql .= $tbl_name . "." . $user_id_col_name . " IN (SELECT user_id FROM " . DUD_WPDB_PREFIX . "usermeta WHERE ((
			" . DUD_WPDB_PREFIX . "usermeta.meta_key = '" . DUD_WPDB_PREFIX . "capabilities' AND (";
	}
	else
	{
